...
describe("E2E - Kliknięcie w Jadłospis po zalogowaniu", () => {
    it("Pobieranie jadłospisu z jego elementami", () => {
       ...
       //Pobieranie kontenera z trzema kartami
       ...
       //Ebook - Keto na start
       cy.contains('.container > :nth-child(1) > a','Ebook - Keto na Start');

       //Pobieranie zakładek "Razem", "Pierwszy partner", "Drugi partner"
       cy.get(".date-menu-box")
       .first()
       .find(".switch-for-couple")
       .first()
       .find("button")
       .then(button => {
    ...
    .then(kontener => {
        expect(kontener).to.have.length(2)
    })
    .find("button")
    .then(button => {
        expect(button).to.have.length(4)
